...
function envo_blog_theme_stylesheets() {
	...
	// Theme stylesheet.
	wp_enqueue_style( 'envo-blog-stylesheet', get_stylesheet_uri(), array('bootstrap'), '1.1.5'  );
...
function envo_blog_theme_js() {
	...
	wp_enqueue_script( 'envo-blog-theme-js', get_template_directory_uri() . '/js/customscript.js', array( 'jquery' ), '1.1.5', true );
}
...
function envo_blog_widgets_init() {
	register_sidebar(
		array(
			'name'			 => esc_html__( 'Homepage Full Width Section #1', 'envo-blog' ),
	...
	register_sidebar(
		array(
			'name'			 => esc_html__( 'Footer Section', 'envo-blog' ),
	...
	<span class="comments-meta">
		<?php
			if ( !comments_open() ) 
				...
				<a href="<?php echo esc_url( get_comments_link() ); ?>" rel="nofollow" title="<?php esc_attr_e( 'Comment on ', 'envo-blog' ) . the_title_attribute(); ?>">
