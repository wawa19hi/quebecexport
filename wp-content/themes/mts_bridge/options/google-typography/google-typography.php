...
class mtsGoogleTypography {
	...
	function help_tab() {
		...
		$adding_title				= __('Adding A Collection', 'bridge' );
		$adding_content				= '<p>'.__('To add a new font for use on your site. Click the "Add New" button on the top left of the page near the "Google Typography" title.', 'bridge' ).'</p>';
		$adding_content				.= '<p>'.__('Once added, a new font row will appear on the page below. Next you can continue to customize your font (more info in the "Customizing" help tab).', 'bridge' ).'</p>';
		//$adding_content			 .= '<p><a href="https://vimeo.com/67957799" target="_blank">'.__('Watch The Video Tutorial &rarr;', 'bridge' ).'</a></p>';
		$customizing_title 		= __('Customizing', 'bridge' );
		$customizing_content	= '<p>'.__('Customizing fonts is easy; after adding a new font row you can then customize the following font attributes:', 'bridge' ).'</p>';
		$customizing_content .= '<ul><li><b>'.__('Preview Text', 'bridge' ).'</b> - '.__('Used for live previewing your changes. This text does not appear anywhere on your website.', 'bridge' ).'</li><li><b>Preview Background Color</b> - Allows you to swap between light and dark backgrounds when previewing this font.</li><li><b>'.__('Font Family', 'bridge' ).'</b> - '.__('The font family to use for this font. Choose from a real-time list of all available Google Fonts.', 'bridge' ).'</li><li><b>'.__('Font Variant', 'bridge' ).'</b> - '.__('The variant to use for this font. Note: Each font has it\'s own variant options.', 'bridge' ).'</li><li><b>'.__('Font Size', 'bridge' ).'</b> - '.__('The size you would like this font to be.', 'bridge' ).'</li><li><b>'.__('Font Color', 'bridge' ).'</b> - '.__('The color you\'d like to use for this font.', 'bridge' ).'</li><li><b>'.__('CSS Selectors', 'bridge' ).'</b> - '.__('The HTML tags or CSS selectors you\'d like this font to apply to (more info in the "CSS Selectors" help tab). You can specify multiple selectors separated by comma\'s. Ex: h1, #some_id, .some_class', 'bridge' ).'</li></ul>';
		$selectors_title 			= __('CSS Selectors', 'bridge' );
		$selectors_content		= '<p>' . __('CSS Selectors are used to hook your font rows into your actual website. Once you\'ve added, customized, and defined CSS selectors for your fonts, Google Typography will automatically insert all the necessary CSS into your website.', 'bridge' ) . '</p>';
		$selectors_content		= '<p>' . __('Here are some examples of the selectors you can use:', 'bridge' ) . '</p>';
		$selectors_content	 .= '<ul><li><b>'.__('IDs', 'bridge' ).':</b> '.__('#selector', 'bridge' ).'</li><li><b>'.__('Classes:', 'bridge' ).':</b> '.__('.selector', 'bridge' ).'</li><li><b>'.__('HTML Tags', 'bridge' ).':</b> '.__('span', 'bridge' ).'</ul>';
		$selectors_content	 .= '<p><b>'.__('Example', 'bridge' ).':</b> '.__('#selector span.date', 'bridge' ).'</p>';
	...
	function plugin_link($links, $file) {
		if ( $file == 'google-typography/google-typography.php' ) {
			$links['settings'] = sprintf( '<a href="%s"> %s </a>', admin_url( 'themes.php?page=typography' ), __( 'Settings', 'bridge' ) );
		}
		return $links;
	}
	...
	function options_ui() {
		...
		$title						= __('Theme Typography', 'bridge' );
		$loading					= __('Loading Your Collections', 'bridge' );
		$add_new					= __('Add New Collection', 'bridge' );
		$reset						= __('Reset Collections', 'bridge' );
		$preview_text   			= __('Type in some text to preview...', 'bridge' );
		$collection_title 			= __('New Collection', 'bridge' );
		//$this->preview_text = $preview_text; // to pass it to JS
		$preview_hint   			= __('Preview Background Color', 'bridge' );
		//$font_family_title  	= __('Font family...', 'bridge' );
		$std_font_family_title  	= __('Standard Fonts', 'bridge' );
		$google_font_family_title  	= __('Google Fonts', 'bridge' );
		$font_variant_title 		= __('Variant...', 'bridge' );
		$font_size_title 			= __('Size...', 'bridge' );
		$backup_font_title			= __('Fallback:', 'bridge' );
		$css_selectors_title		= __('CSS Selectors:', 'bridge' );
		$additional_css_title		= __('Additional CSS:', 'bridge' );
		$delete_button_text			= __('Delete', 'bridge' );
		$save_button_text			= __('Save', 'bridge' );
		
		$welcome_title 				= __('Theme Typography', 'bridge' );
		$welcome_subtitle			= __('Get started in 3 steps.', 'bridge' );//.'<a href="https://vimeo.com/67957799" target="_blank">'.__('Watch the video tutorial &#x2192;', 'bridge' ).'</a>';
		$step_1_title 				= __('1. Pick A Font', 'bridge' );
		$step_1_desc				= __('Choose from any of the 600+ Google Fonts.', 'bridge' );
		$step_2_title 				= __('2. Customize It', 'bridge' );
		$step_2_desc				= __('Pick a size, variant, color and more.', 'bridge' );
		$step_3_title 				= __('3. Attach It', 'bridge' );
		$step_3_desc				= __('Attach your font to any CSS selector(s).', 'bridge' );
		$year						= date("Y");
		$themename = MTS_THEME_NAME;
		
		$character_sets_title = __('Character sets', 'bridge' );
		$character_sets_desc = __('Choose the character sets you wish to include. Please note that not all sets are available for all fonts.', 'bridge' );
		...
		$character_sets = array(
			'latin' 		=> __('Latin', 'bridge' ),
			'latin-ext' 	=> __('Latin Extended', 'bridge' ),
			'cyrillic' 		=> __('Cyrillic', 'bridge' ),
			'cyrillic-ext'	=> __('Cyrillic Extended', 'bridge' ),
			'greek'			=> __('Greek', 'bridge' ),
			'greek-ext'		=> __('Greek Extended', 'bridge' ),
			'vietnamese'	=> __('Vietnamese', 'bridge' ),
			'khmer'			=> __('Khmer', 'bridge' ),
			'devanagari'	=> __('Devanagari', 'bridge' ),
		);
...
EOT;
	...
	function admin_scripts() {
		...
		wp_localize_script('google-typography', 'googletypography', array(
				'reset_confirm' => __("Are you sure you want to revert to the theme's default collections? Note: You will lose any custom collections you've created.", 'bridge' ),
				'delete_confirm' => __("Are you sure you want to delete this collection?", 'bridge' )
			));
