	...
	*/
	function render(){
		...
		$css_options = array(
			'repeat'	 => array(
				''		  => __('Default', 'bridge' ),
				'no-repeat' => __('No Repeat', 'bridge' ),
				'repeat'	=> __('Repeat All', 'bridge' ),
				'repeat-x'  => __('Repeat Horizontally', 'bridge' ),
				'repeat-y'  => __('Repeat Vertically', 'bridge' ),
				//'inherit'   => 'Inherit',
			),
			'attachment' => array(
				''		=> __('Default', 'bridge' ),
				'fixed'   => __('Fixed', 'bridge' ),
				'scroll'  => __('Scroll', 'bridge' ),
				//'inherit' => 'Inherit',
			),
			'position'   => array(
				''			  => __('Default', 'bridge' ),
				'left top'	  => __('Left Top', 'bridge' ),
				'left center'   => __('Left center', 'bridge' ),
				'left bottom'   => __('Left Bottom', 'bridge' ),
				'center top'	=> __('Center Top', 'bridge' ),
				'center center' => __('Center Center', 'bridge' ),
				'center bottom' => __('Center Bottom', 'bridge' ),
				'right top'	 => __('Right Top', 'bridge' ),
				'right center'  => __('Right center', 'bridge' ),
				'right bottom'  => __('Right Bottom', 'bridge' ),
			),
			'size'	   => array(
				''		=> __('Default', 'bridge' ),
				//'inherit' => 'Inherit',
				'cover'   => __('Cover', 'bridge' ),
				'contain' => __('Contain', 'bridge' ),
			),
			'parallax'   => array(
				'0' => __('Off', 'bridge' ),
				'1' => __('On', 'bridge' ),
		...
		if ( $this->field['options']['color'] !== false ) {
			...
			echo '<div class="bg-opt-input-label">'.__( 'Background Color:', 'bridge' ).'</div>';
		...
		// Tabs
		echo '<div class="bg-opt-input-label">'.__( 'Background Image:', 'bridge' ).'</div>';
		echo '<fieldset class="green buttonset buttonset-tabs">';
			if ( $this->field['options']['image_pattern'] !== false ) {
				...
				echo '<label id="nhp-opts-button" for="'.$this->field['id'].'_pattern" class="buttonset-tab">'.__('Pattern', 'bridge' ).'</label>';
			}
			if ( $this->field['options']['image_upload'] !== false ) {
				...
				echo '<label id="nhp-opts-button" for="'.$this->field['id'].'_upload" class="buttonset-tab">'.__('Upload', 'bridge' ).'</label>';
			}
			if ( $this->field['options']['gradient'] !== false ) {
				...
				echo '<label id="nhp-opts-button" for="'.$this->field['id'].'_gradient" class="buttonset-tab">'.__('Gradient', 'bridge' ).'</label>';
			}
		...
		if ( $this->field['options']['image_upload'] !== false ) {
			...
			echo ' <a href="javascript:void(0);" class="nhp-opts-upload button-secondary"'.$upload.' rel-id="'.$this->field['id'].'_image_upload">'.__('Browse', 'bridge' ).'</a>';
			echo ' <a href="javascript:void(0);" class="nhp-opts-upload-remove"'.$remove.' rel-id="'.$this->field['id'].'_image_upload">'.__('Remove Upload', 'bridge' ).'</a>';
			echo '</fieldset>';
			...
			if ( $this->field['options']['repeat'] !== false ) {
				...
				echo '<label for="'.$this->field['id'].'_repeat" class="bg-opt-input-label">'.__( 'Background Repeat:', 'bridge' ).'</label>';
			...
			if ( $this->field['options']['attachment'] !== false ) {
				...
				echo '<div class="bg-upload-select">';
				echo '<label for="'.$this->field['id'].'_attachment" class="bg-opt-input-label">'.__( 'Background Attachment:', 'bridge' ).'</label>';
			...
			if ( $this->field['options']['position'] !== false ) {
				...
				echo '<div class="bg-upload-select">';
				echo '<label for="'.$this->field['id'].'_position" class="bg-opt-input-label">'.__( 'Background Position:', 'bridge' ).'</label>';
			...
			if ( $this->field['options']['size'] !== false ) {
				...
				echo '<div class="bg-upload-select">';
				echo '<label for="'.$this->field['id'].'_size" class="bg-opt-input-label">'.__( 'Background Size:', 'bridge' ).'</label>';
		...
		if ( $this->field['options']['gradient'] !== false ) {
			...
			echo '<div class="bg-opt-input-label">' . __('From:', 'bridge' ) . '</div><input type="text" id="'.$this->field['id'].'_gradient_from" name="'.$this->args['opt_name'].'['.$this->field['id'].'][gradient][from]" value="'.$this->value['gradient']['from'].'" class="popup-colorpicker" data-default-color="'.$defaults['gradient']['from'].'" style="width:70px;"/>';
			echo '</div>';
			echo '<div class="color-gradient-step-wrapper">';
			echo '<div class="bg-opt-input-label">' . __('To:', 'bridge' ) . '</div><input type="text" id="'.$this->field['id'].'_gradient_to" name="'.$this->args['opt_name'].'['.$this->field['id'].'][gradient][to]" value="'.$this->value['gradient']['to'].'" class="popup-colorpicker" data-default-color="'.$defaults['gradient']['to'].'" style="width:70px;"/>';
			echo '</div>';

			echo '<div class="color-gradient-direction-wrapper">';
			echo '<label for="'.$this->field['id'].'_gradient_direction" class="bg-opt-input-label">' . __('Direction:', 'bridge' ) . '</label>';
			echo '<select id="'.$this->field['id'].'_gradient_direction" name="'.$this->args['opt_name'].'['.$this->field['id'].'][gradient][direction]" class="color-gradient-direction">';
				echo '<option value="horizontal"' . selected( $this->value['gradient']['direction'], 'horizontal', false ) . '>' . __( 'Horizontal', 'bridge' ) . '</option>';
				echo '<option value="vertical"' . selected( $this->value['gradient']['direction'], 'vertical', false ) . '>' . __( 'Vertical', 'bridge' ) . '</option>';
			echo '</select>';
			echo '</div>';
			echo '</div>';
			echo '</div>';
		...
		if ( $this->field['options']['parallax'] !== false ) {
			...
			echo '<div class="bg-opt-input-label">'.__( 'Parallax Effect:', 'bridge' ).'</div>';
