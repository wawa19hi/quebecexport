	...
	*/
	function render(){
		if (empty($this->value) || !is_array($this->value)) {
			$this->value = array(
				array(
					'group_title' => __('New', 'bridge' ).' '.$this->field['groupname'],
					'group_sort' => '0',
				)
			);
		}
		...
		echo '<div class="nhp-opts-groups-accordion-group nhp-opts-dummy" style="display:none" id="nhp-opts-dummy-' . $this->field['id'] . '"><h3><span class="nhp-opts-groups-header">' . __("New ", 'bridge') . $this->field['groupname'] . '</span></h3>';
		...
		$field_is_title = true;
			...
			echo '</table>';
			echo '<a href="javascript:void(0);" class="button button-secondary nhp-opts-groups-close">' . __('OK', 'bridge' ). '</a>';
			echo '<a href="javascript:void(0);" class="button deletion nhp-opts-groups-remove">' . __('Delete', 'bridge' ).' '. $this->field['groupname']. '</a>';
			echo '</div></div>';
		...
		foreach ($groups as $k => $group) {
			...
			//echo '<h4>' . __('Group Title', 'bridge' ) . '</h4>';
		...
		echo '</div><a href="javascript:void(0);" class="button nhp-opts-groups-add button-secondary" rel-id="' . $this->field['id'] . '-ul" rel-name="' . $this->parent->args['opt_name'] . '[' . $this->field['id'] . '][group_title][]">' . __('Add', 'bridge' ) .' '.$this->field['groupname']. '</a><br/>';
