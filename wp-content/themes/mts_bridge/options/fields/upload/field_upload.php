...
class NHP_Options_upload extends NHP_Options{	
	...
	*/
	function render(){
		...
		echo ' <a href="javascript:void(0);" class="nhp-opts-upload button-secondary"'.$upload.' rel-id="'.$this->field['id'].'">'.__('Browse', 'bridge' ).'</a>';
		echo ' <a href="javascript:void(0);" class="nhp-opts-upload-remove"'.$remove.' rel-id="'.$this->field['id'].'">'.__('Remove Upload', 'bridge' ).'</a>';
	...
	*/
	function enqueue(){
		...
		wp_localize_script('nhp-opts-field-upload-js', 'nhp_upload', array(
			...
			'title' => __( 'Select Image', 'bridge' ),
			'buttonText' => __( 'Select Image', 'bridge' )
		));
