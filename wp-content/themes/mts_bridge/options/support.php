...
class MTS_Options_Tab_Support {
	...
	public function add_support_tab( $sections ) {
		...
		$sections['support'] = array(
			...
			sprintf( __( 'We offer technical support through our %s. Please <strong>copy and paste the following information in your ticket</strong> when contacting support:' , 'bridge' ), '<a href="https://community.mythemeshop.com/forum/7-theme-support/" target="_blank">' . __( 'Support Ticket System', 'bridge' ) . '</a>' )
			 . '</p>',
	...
	public function add_support_tab_content() {
		...
		<div class="mts-options-extra-tab-content">
			...
			<textarea class="large-text" id="mts-debug-data-field" rows="16" readonly="readonly" disabled><?php _e( 'Generating log. Please wait...', 'bridge' ); ?></textarea>
	...
	public function support_buttons() {
		...
		<div class="mts-help-buttons">
			<button type="button" class="button mts-support-copy" disabled>
				...
				<?php _e( 'Copy Data for Support Request', 'bridge' ); ?>
			</button> 
			<a href="https://community.mythemeshop.com/forum/7-theme-support/" target="_blank" class="button button-primary mts-support-link">
				...
				<?php _e( 'Open Support Forum', 'bridge' ); ?>
			</a>
		</div>
		<?php
	}
	...
	public function get_debug_data() {
		...
		$data['environment'] = array(
			'title' => __( 'Environment', 'bridge' ),
			'data' => $this->get_environment_info(),
		);

		$data['wp'] = array(
			'title' => __( 'WordPress', 'bridge' ),
			'data' => $this->get_wp_info(),
		);

		$data['plugins'] = array(
			'title' => __( 'Plugins', 'bridge' ),
			'data' => $this->get_plugins_info(),
		);

		$data['themes'] = array(
			'title' => __( 'Theme', 'bridge' ),
			'data' => $this->get_theme_info(),
		);

		$data['settings'] = array(
			'title' => __( 'Theme Settings', 'bridge' ),
