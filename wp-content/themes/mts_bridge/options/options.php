...
class NHP_Options{
	...
	*/
	function __construct($sections = array(), $args = array(), $extra_tabs = array()){
		...
		$defaults['menu_title'] = __('Options', 'bridge' );
		$defaults['page_icon'] = 'icon-themes';
		$defaults['page_title'] = __('Options', 'bridge' );
	...
	*/
	function _enqueue(){
		...
		wp_localize_script( 'nhp-opts-js', 'nhpopts', array(
			...
			'reset_confirm' => __('Are you sure you want to reset ALL options to default?', 'bridge' ),
			'import_confirm' => __('Are you sure you want to import options? All current options will be lost.', 'bridge' ),
			'leave_page_confirm' => __('Settings have changed, you should save them! Are you sure you want to leave this page?', 'bridge' ),
			'child_theme_name_empty' => __('Please enter desired child theme name.', 'bridge' ),
			'import_done' => __('Importing proccess finished!', 'bridge' ),
			'import_fail' => __('Importing proccess failed! Please try again.', 'bridge' ),
			'remove_done' => __('Removal proccess finished!', 'bridge' ),
			'remove_fail' => __('Removal proccess failed! Please try again.', 'bridge' ),
			'reloading_page' => __('Reloading page...', 'bridge' ),
			'import_opt_confirm' => __('Are you sure you want to import demo options? All current options will be lost.', 'bridge' ),
			'import_widget_confirm' => __('Are you sure you want to import demo options and widgets? All current options will be lost and existing widgets deactivated.', 'bridge' ),
			'import_all_confirm' => __('Are you sure you want to import demo?', 'bridge' ),
			'remove_all_confirm' => __("Are you sure you want to remove demo options, content, menus and widgets? Please note that all modifications you've made to imported content will be lost", 'bridge' ),
		) );
	...
	*/
	function _download_options(){
		if(!isset($_GET['secret']) || $_GET['secret'] != md5(AUTH_KEY.SECURE_AUTH_KEY)){wp_die(__( 'Invalid Secret for options use', 'bridge' ));exit;}
		if(!isset($_GET['option'])){wp_die(__( 'No Option Defined', 'bridge' ));exit;}
	...
	function _options_page_html(){
		...
		echo '<div class="wrap"><h2></h2>';
			...
			echo '<form method="post" action="options.php" enctype="multipart/form-data" id="nhp-opts-form-wrapper">';
				...
				if ( ! MTS_THEME_WHITE_LABEL ) {
					echo '<a href="https://community.mythemeshop.com/forum/7-theme-support/" class="docsupport" target="_blank"><i class="fa fa-medkit"></i> '.__('Support', 'bridge').'</a>';
				}
					echo '<div class="theme-options-search"><a href="#" class="theme-options-clear-search"><i class="dashicons dashicons-no"></i></a> <input type="text" id="search-theme-options" value="" placeholder="'.esc_attr__('Search Options', 'bridge').'"><a href="#" class="options-search-link"><i class="fa fa-search"></i><span> '.__('Search Options', 'bridge').'</span></a></div>';
				...
				echo '</div>';
					...
					if(isset($_GET['settings-updated']) && $_GET['settings-updated'] == 'true' && get_transient('nhp-opts-saved') == '1'){
						if(isset($this->options['imported']) && $this->options['imported'] == 1){
							echo '<div id="nhp-opts-imported"><strong>'.__('Settings Imported!', 'bridge' ).'</strong></div>';
						}else{
							echo '<div id="nhp-opts-save"><strong>'.__('Settings Saved!', 'bridge' ).'</strong></div>';
						}
						delete_transient('nhp-opts-saved');
					}
					echo '<div id="nhp-opts-save-warn"><strong>'.__('Settings have changed, you should save them!', 'bridge' ).'</strong></div>';
					echo '<div id="nhp-opts-field-errors"><strong><span></span> '.__('error(s) were found!', 'bridge' ).'</strong></div>';

					echo '<div id="nhp-opts-field-warnings"><strong><span></span> '.__('warning(s) were found!', 'bridge' ).'</strong></div>';
					...
					echo '<ul id="nhp-opts-group-menu">';
						...
						if(true === $this->args['show_typography']){
							echo '<li id="typography_default_section_group_li" class="nhp-opts-group-tab-link-li">';
									echo '<a href="javascript:void(0);" id="typography_default_section_group_li_a" class="nhp-opts-group-tab-link-a" data-rel="typography_default"><i class="fa fa-text-width"></i> '.'<span class="section_title">'.__('Typography', 'bridge' ).'</span></a>';
							echo '</li>';
						...
						if(true === $this->args['show_import_export']){
							echo '<li id="import_export_default_section_group_li" class="nhp-opts-group-tab-link-li">';
									echo '<a href="javascript:void(0);" id="import_export_default_section_group_li_a" class="nhp-opts-group-tab-link-a" data-rel="import_export_default"><i class="fa fa-sign-in"></i> '.'<span class="section_title">'.__('Import / Export', 'bridge' ).'</span></a>';
							echo '</li>';
						...
						if(true === $this->args['dev_mode']){
							echo '<li id="dev_mode_default_section_group_li" class="nhp-opts-group-tab-link-li">';
									echo '<a href="javascript:void(0);" id="dev_mode_default_section_group_li_a" class="nhp-opts-group-tab-link-a custom-tab" data-rel="dev_mode_default"><img src="'.$this->url.'img/glyphicons/glyphicons_195_circle_info.png" /> '.__('Dev Mode Info', 'bridge' ).'</a>';
							echo '</li>';
						}//if
				...
				echo '<div id="nhp-opts-main">';
					...
					if(true === $this->args['show_import_export']){
						echo '<div id="import_export_default_section_group'.'" class="nhp-opts-group-tab">';
							echo '<h2>'.__('Import / Export Options', 'bridge' ).'</h2>';

							// presets
							if (!empty($this->args['presets'])) {

								echo '<h4>'.__('Preset Options', 'bridge' ).'</h4>';
								...
								foreach ( $this->args['presets'] as $key => $preset ) {
									echo '<div class="preset" data-nonce="'.$nonce.'" data-demo-id="'.esc_attr($key).'">';
										$imported_flag	= '<span class="demo_part_imported">'.__( '(imported)', 'bridge' ).'</span>';
										...
										if ( ! empty( $preset['demo'] ) ) echo '<a href="'.$preset['demo'].'" target="_blank" class="demo-link">'.__('View demo', 'bridge' ).'</a>';
										...
										echo '<div class="preset-actions-wrap">';
											echo '<button class="button button-secondary import-demo-options-button"'.$disabled_button.'>'.__('Import Theme Options', 'bridge' ).'</button>';
											echo '<button class="button button-secondary import-demo-widgets-button"'.$disabled_button.'>'.__('Import Theme Options & Widgets', 'bridge' ).'</button>';
											if ( !empty( $all_imported ) ) {
												echo '<button class="button button-primary remove-demo-button" data-nonce="'.$nonce.'">'.__('Remove imported data', 'bridge' ).'</button>';
											} else {
												echo '<button class="button button-secondary import-demo-button"'.$disabled_button.'>'.__('Import Theme Options, Widgets & Content', 'bridge' ).'</button>';
											}
										echo '</div>';
								...
								echo '<div id="importing-overlay" class="mfp-hide">';
									echo '<div id="importing-modal">';
										echo '<div id="importing-modal-header">';
											echo '<h2><span class="spinner is-active"></span>' . __( 'Processing, please wait...', 'bridge' ) . '</h2>';
										echo '</div>';
										echo '<div id="importing-modal-content">';
										echo '</div>';
										echo '<div id="importing-modal-footer">';
											echo '<span id="importing-modal-footer-info">'.__( 'Processing, please wait...', 'bridge' ).'</span>';
											echo '<button id="importing-modal-footer-button" class="button button-primary">'.__( 'Ok', 'bridge' ).'</button>';
										echo '</div>';
									echo '</div>';
								echo '</div>';
							...
							echo '<h4>'.__('Import Options', 'bridge' ).'</h4>';

							echo '<p><a href="#" id="nhp-opts-import-code-button" class="button-secondary">' . esc_html__( 'Import Code', 'bridge' ) . '</a></p>';

							echo '<div id="nhp-opts-import-code-wrapper">';

								echo '<div class="nhp-opts-section-desc">';

									echo '<p class="description" id="import-code-description">'.apply_filters('nhp-opts-import-file-description',__('Insert your backup code below and hit Import to restore your site options from a backup.', 'bridge' )).'</p>';
								...
								echo '<div class="nhp-opts-field-wrapper">';
									...
									echo '<input type="submit" id="nhp-opts-import" name="'.$this->args['opt_name'].'[import]" class="button-primary" value="'.__('Import', 'bridge' ).'">';
							...
							if( true === $this->args['show_child_theme_opts'] && !is_child_theme() ) {
								...
								echo '<h4>'.__('Child Theme', 'bridge' ).'</h4>';
								echo '<div class="nhp-opts-section-desc">';
									echo '<p class="description">'. __('To create child theme for current theme, enter desired name and hit "Create Child Theme" button', 'bridge' ).'</p>';
								echo '</div>';

								echo '<div class="nhp-opts-field-wrapper">';
									//echo '<label for="nhp-opts-child-name">'.__( 'Child theme name', 'bridge' ).'</label><br />';
									echo '<input type="text" id="nhp-opts-child-name" name="nhp-opts-child-name" value="" />';
									echo '<input type="submit" id="nhp-opts-child-button" name="nhp-opts-child-button" class="button-primary" value="'.__('Create Child Theme', 'bridge' ).'">';
								echo '</div>';
					...
					if(true === $this->args['show_typography']){
						echo '<div id="typography_default_section_group'.'" class="nhp-opts-group-tab">';
							//echo '<h3>'.__('Theme Typography', 'bridge' ).'</h3>';
					...
					if(true === $this->args['dev_mode']){
						echo '<div id="dev_mode_default_section_group'.'" class="nhp-opts-group-tab">';
							echo '<h2>'.__('Dev Mode Info', 'bridge' ).'</h2>';
							echo '<div class="nhp-opts-section-desc">';
					...
					echo '<div id="options-search-no-results">'.__('No options found, please refine your query.', 'bridge' ).'</div><!--search: no results-->';
				...
				echo '<div id="nhp-opts-footer">';
					...
					if ( ! empty( $this->sections ) && count( $this->sections ) > 2 ) {
						echo '<input type="submit" name="'.$this->args['opt_name'].'[defaults]" value="'.__('Reset to Defaults', 'bridge' ).'" class="button-secondary" />';
						echo '<input type="submit" name="save" id="savechanges" value="'.__('Save Changes', 'bridge' ).'" class="button-primary" disabled="disabled" />';
					}
					echo '<div class="clear"></div><!--clearfix-->';
				echo '</div>';
	...
	*/
	...
	function mts_filesystem_notice() {
		echo '<div class="updated"><p>' . esc_html__( 'You do not have permission to do this.', 'bridge' ) . '</p></div>';
	}
	...
	function mts_list_child_themes() {
		...
		if ( !empty( $child_themes_arr ) ) {
			echo '<p class="description">' . __('Existing child themes for current theme:', 'bridge' ) . '</p>';
			...
			_e('No child themes found for current theme', 'bridge' );
		}
		echo '</div>';
