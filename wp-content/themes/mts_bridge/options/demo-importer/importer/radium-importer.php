	...
	class Radium_Theme_Importer {
		...
		public function set_demo_data( $file ) {
			...
			if ( $importer_error ) {
				...
				mts_chunk_output( __( 'Error on import.', 'bridge' ), 'h4' );
				die();
				...
				if(!is_file( $file )){
					...
					mts_chunk_output(__( "The XML file containing the dummy content is not available or could not be read .. You might want to try to set the file permission to chmod 755.<br/>If this doesn't work please use the Wordpress importer and import the XML file (should be located in your download .zip: Sample Content folder) manually.", 'bridge' ) );
		...
		public function set_demo_theme_options( $file ) {
			...
			if ( ! file_exists( $file ) ) {
				wp_die(
					__( 'Theme options Import file could not be found. Please try again.', 'bridge' ),
					'',
			...
			if ( $this->isSerialized( $data ) ) {
				...
				if ( !empty( $data ) || is_array( $data ) ) {
				 	...
				 	mts_chunk_output( __('Importing options...', 'bridge' ), 'h4' );
					// Hook before import
					$data = apply_filters( 'radium_theme_import_theme_options', $data );

					update_option( $this->theme_option_name, $data );

					mts_chunk_output( __('Options imported!', 'bridge' ), 'h4' );
					
				} else {

					mts_chunk_output( __( 'No Options to import.', 'bridge' ), 'h4' );
				}
			} else {
				mts_chunk_output( __( 'Something is wrong with options data. Please contact the support.', 'bridge' ), 'h4' );
			}
		}
		...
		function process_widget_import_file( $file ) {
			...
			if ( ! file_exists( $file ) ) {
				wp_die(
					__( 'Widget Import file could not be found. Please try again.', 'bridge' ),
					'',
					array( 'back_link' => true )
				);
			}
			...
			mts_chunk_output( __( 'Importing widgets...', 'bridge' ), 'h4' );
			...
			mts_chunk_output( __( 'Widgets imported!', 'bridge' ), 'h4' );
		}
		...
		public function import_widgets( $data ) {
			...
			foreach ( $data as $sidebar_id => $widgets ) {
				...
				if ( isset( $wp_registered_sidebars[$sidebar_id] ) ) {
					...
					$sidebar_message = __( 'Sidebar does not exist in theme (using Inactive)', 'bridge' );
				}
				...
				foreach ( $widgets as $widget_instance_id => $widget ) {
					...
					if ( ! $fail && ! isset( $available_widgets[$id_base] ) ) {
						...
						$widget_message = __( 'Site does not support widget', 'bridge' ); // explain why widget not imported
					}
					...
					if ( ! $fail && isset( $widget_instances[$id_base] ) ) {
						...
						foreach ( $single_widget_instances as $check_id => $check_widget ) {
							...
							if ( in_array( "$id_base-$check_id", $sidebar_widgets ) && (array) $widget == $check_widget ) {
								...
								$widget_message = __( 'Widget already exists', 'bridge' ); // explain why widget not imported
					...
					if ( ! $fail ) {
						...
						if ( $sidebar_available ) {
							...
							$widget_message = __( 'Imported', 'bridge' );
						} else {
							$widget_message_type = 'warning';
							$widget_message = __( 'Imported to Inactive', 'bridge' );
						}
					}

					// Result for widget instance
					$results[$sidebar_id]['widgets'][$widget_instance_id]['name'] = isset( $available_widgets[$id_base]['name'] ) ? $available_widgets[$id_base]['name'] : $id_base; // widget name or ID if name not available (not supported by site)
					$results[$sidebar_id]['widgets'][$widget_instance_id]['title'] = property_exists( $widget, 'title' ) && !empty( $widget->title ) ? $widget->title : __( 'No Title', 'bridge' ); // show "No Title" if widget instance is untitled
		...
		public function make_widgets_inactive() {
			...
			mts_chunk_output( __( 'Deactivating current widgets...', 'bridge' ), 'h4' );
			...
			if ( $sidebars_widgets && is_array( $sidebars_widgets ) && !empty( $sidebars_widgets ) ) {
				...
				if ( !empty( $widgets_to_move ) ) {
					...
					update_option( 'sidebars_widgets', $sidebars_widgets );
					mts_chunk_output( __( 'Widgets deactivated.', 'bridge' ), 'h4' );
				} else {
					mts_chunk_output( __( 'No widgets to deactivate.', 'bridge' ), 'h4' );
				}
			}
		}
		...
		public function backup_widgets() {
			...
			if ( !isset( $imported_demos['widgets'] ) ) {
				...
				if ( $sidebars_widgets ) {
					...
					mts_chunk_output( __( 'Created sidebars backup.', 'bridge' ), 'h4' );
				}
			}
		}
		...
		public function backup_options() {
			...
			if ( !isset( $imported_demos['options'] ) ) {
				...
				if ( $mts_options ) {
					...
					mts_chunk_output( __( 'Created options backup.', 'bridge' ), 'h4' );
				}
			}
		}
		...
		public function restore_widgets() {
			...
			if ( $sidebars ) {
				...
				mts_chunk_output( __( 'Sidebars restored.', 'bridge' ), 'h4' );
			}
		}
		...
		public function restore_options() {
			...
			if ( $options ) {
				...
				mts_chunk_output( __( 'Options restored.', 'bridge' ), 'h4' );
			}
		}
		...
		public function clear_menu_locations() {
			...
			if ( $menus ) {
				...
				mts_chunk_output( __( 'Menu Locations cleared.', 'bridge' ), 'h4' );
			}
		}
