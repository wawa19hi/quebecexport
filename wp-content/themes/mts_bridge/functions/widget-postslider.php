	...
	class mts_post_slider_widget extends WP_Widget {
		...
		public function __construct() {
			parent::__construct(
				...
				sprintf( __('%sPost Slider', 'bridge' ), MTS_THEME_WHITE_LABEL ? '' : 'MTS ' ),
				array( 'description' => __( 'Display posts from multiple categories in an animated slider.', 'bridge' ) )
			);
		}

		public function form( $instance ) {
			$defaults = array(
				'title' => __( 'Featured Posts', 'bridge' ),
				'cat' => array(),
				'slides_num' => 3,
				'show_title' => 0,
				'slider_nav' => 'bullets',
			...
			<p>
				<label for="<?php echo $this->get_field_id( 'slides_num' ); ?>"><?php _e( 'Number of Posts to show', 'bridge' ); ?></label>
			...
			<p>
				<label for="<?php echo $this->get_field_id( 'slider_nav' ); ?>"><?php _e( 'Navigaton type:', 'bridge' ); ?></label>
				<select id="<?php echo $this->get_field_id( 'slider_nav' ); ?>" name="<?php echo $this->get_field_name( 'slider_nav' ); ?>" class="widefat">
					<option value="bullets"<?php selected( $instance['slider_nav'], 'bullets' ); ?>><?php _e( 'Bulleted', 'bridge' ); ?></option>
					<option value="arrows"<?php selected( $instance['slider_nav'], 'arrows' ); ?>><?php _e( 'Arrows', 'bridge' ); ?></option>
				</select>
			</p>
						...
						<div class="widget-slider widget-slider-<?php echo $slider_nav; ?>">
							...
							<div class="slide">
								<a href="<?php echo esc_url( get_the_permalink() ); ?>">
									<?php the_post_thumbnail('bridge-widgetfull',array('title' => '')); ?>
									<?php if ( $show_title ) { ?>
