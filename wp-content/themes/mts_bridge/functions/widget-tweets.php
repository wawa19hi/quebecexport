	...
	class mts_widget_recent_tweets extends WP_Widget {
		public function __construct() {
			parent::__construct(
				...
				sprintf( __( '%sLatest Tweets', 'bridge' ), MTS_THEME_WHITE_LABEL ? '' : 'MTS ' ),// Name
				array( 'description' => __( 'A widget for showing latest tweets in sidebar', 'bridge' ), ) // Args
			);
		}
		public function widget($args, $instance) {
			...
			if ( empty( $tweets ) ) { _e( 'Couldn\'t retrieve tweets! Wrong username?', 'bridge' ); echo $after_widget; return; }
			if(!empty($tweets->errors)){
				if($tweets->errors[0]->message == 'Invalid or expired token'){
					echo '<strong>'.$tweets->errors[0]->message.'!</strong><br />'. esc_html( 'You\'ll need to regenerate it <a href="https://dev.twitter.com/apps" target="_blank">here</a>!', 'bridge' ) . $after_widget;
		...
		echo '<p><label>Title:</label><input type="text" name="'.$this->get_field_name( 'title' ).'" id="'.$this->get_field_id( 'title' ).'" value="'.esc_attr($instance['title']).'" class="widefat" /></p><p><label>'.__('API Key', 'bridge' ).':</label><input type="text" name="'.$this->get_field_name( 'consumerkey' ).'" id="'.$this->get_field_id( 'consumerkey' ).'" value="'.esc_attr($instance['consumerkey']).'" class="widefat" /></p><p><label>'.__('API Secret', 'bridge' ).':</label><input type="text" name="'.$this->get_field_name( 'consumersecret' ).'" id="'.$this->get_field_id( 'consumersecret' ).'" value="'.esc_attr($instance['consumersecret']).'" class="widefat" /></p><p><label>'.__('Access Token', 'bridge' ).':</label><input type="text" name="'.$this->get_field_name( 'accesstoken' ).'" id="'.$this->get_field_id( 'accesstoken' ).'" value="'.esc_attr($instance['accesstoken']).'" class="widefat" /></p>
		<p><label>'.__('Access Token Secret', 'bridge' ).':</label><input type="text" name="'.$this->get_field_name( 'accesstokensecret' ).'" id="'.$this->get_field_id( 'accesstokensecret' ).'" value="'.esc_attr($instance['accesstokensecret']).'" class="widefat" /></p><p><label>'.__('Cache Tweets in every', 'bridge' ).':</label><input type="text" name="'.$this->get_field_name( 'cachetime' ).'" id="'.$this->get_field_id( 'cachetime' ).'" value="'.esc_attr($instance['cachetime']).'" class="small-text" /> hours</p><p><label>'.__('Twitter Username', 'bridge' ).'</label>
		<input type="text" name="'.$this->get_field_name( 'username' ).'" id="'.$this->get_field_id( 'username' ).'" value="'.esc_attr($instance['username']).'" class="widefat" /></p><p><label>'.__('Number of tweets (max 20)', 'bridge' ).'</label><select type="text" name="'.$this->get_field_name( 'tweetstoshow' ).'" id="'.$this->get_field_id( 'tweetstoshow' ).'">';
		...
		public function relative_time($a) {			
			...
			$week = $day * 7;				
			if(is_numeric($d) && $d > 0) {				
				if($d < 3) return __( "right now", 'bridge' ); //if less then 3 seconds
				if($d < $minute) return sprintf( __("%s seconds ago", 'bridge' ), floor($d) ); //if less then minute
				if($d < $minute * 2) return __( "about 1 minute ago", 'bridge' ); //if less then 2 minutes
				if($d < $hour) return sprintf( __( "%s minutes ago", 'bridge' ), floor( $d / $minute ) ); //if less then hour
				if($d < $hour * 2) return __( "about 1 hour ago", 'bridge' ); //if less then 2 hours
				if($d < $day) return sprintf( __("%s hours ago", 'bridge' ), floor( $d / $hour ) ); //if less then day
				if($d > $day && $d < $day * 2) return __( "yesterday", 'bridge' ); //if more then day, but less then 2 days
				if($d < $day * 365) return sprintf(  __( "%s days ago", 'bridge' ), floor( $d / $day ) ); //if less then year
				return __( "over a year ago", 'bridge' ); //else return more than a year
			}
		}
	}	
}	
