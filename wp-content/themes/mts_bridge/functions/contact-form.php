	...
	class mtscontact {
		...
		public function ajax_mtscontact() {
			if ($this->validate()) {
				if ($this->send_mail()) {
					...
					$this->errors['sendmail'] = __('An error occurred. Please contact site administrator.', 'bridge' );
		...
		private function validate() {
			...
			if (!check_ajax_referer( 'mtscontact', 'mtscontact_nonce', false )) {
				$this->errors['nonce'] = __('Please try again.', 'bridge' );
			}
			
			// check honeypot // must be empty
			if (!empty($_POST['mtscontact_captcha'])) {
				$this->errors['captcha'] = __('Please try again.', 'bridge' );
			}
			
			// name field
			$name = trim(str_replace(array("\n", "\r", "<", ">"), '', strip_tags($_POST['mtscontact_name'])));
			if (empty($name)) {
				$this->errors['name'] = __('Please enter your name.', 'bridge' );
			}
			
			// email field
			$useremail = trim($_POST['mtscontact_email']);
			if (!is_email($useremail)) {
				$this->errors['email'] = __('Please enter a valid email address.', 'bridge' );
			}
			
			// message field
			$message = strip_tags($_POST['mtscontact_message']);
			if (empty($message)) {
				$this->errors['message'] = __('Please enter a message.', 'bridge' );
			}
		...
		private function send_mail() {
			...
			$email_subject = __('Contact Form Message from', 'bridge' ).' '.get_bloginfo('name');
			$email_message = __('Name:', 'bridge' ).' '.$this->userinput['name']."\n\n".
							 __('Email:', 'bridge' ).' '.$this->userinput['email']."\n\n".
							 __('Message:', 'bridge' ).' '.$this->userinput['message'];
		...
		public function get_form() {
			...
			if (!$this->success) {
				...
				<div class="mtscontact_consent">
					...
					<label for="mtscontact_consent">'.__('Please tick to agree to be contacted.', 'bridge').'</label>
				</div>

				<input type="submit" value="'.__('Send', 'bridge' ).'" id="mtscontact_submit" />
			</form>';
			}
			$return .= '<div id="mtscontact_success"'.($this->success ? '' : ' style="display: none;"').'>'.__('Your message has been sent.', 'bridge' ).'</div>';
			return $return;
		}
