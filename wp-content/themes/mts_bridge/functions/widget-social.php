	...
	class Social_Profile_Icons_Widget extends WP_Widget {
		...
		function __construct() {
			...
			$widget_ops = array(
				...
				'description' => __( 'Show profile icons.', 'bridge' ),
			);
			...
			parent::__construct( 'social-profile-icons', sprintf( __( '%sSocial Profile Icons', 'bridge' ), MTS_THEME_WHITE_LABEL ? '' : 'MTS ' ), $widget_ops, $control_ops );
		...
		function form( $instance ) {
			...
			<p><label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php _e( 'Title:', 'bridge' ); ?></label> <input class="widefat" id="<?php echo $this->get_field_id( 'title' ); ?>" name="<?php echo $this->get_field_name( 'title' ); ?>" type="text" value="<?php echo esc_attr( $instance['title'] ); ?>" /></p>

			<p><label><input id="<?php echo $this->get_field_id( 'new_window' ); ?>" type="checkbox" name="<?php echo $this->get_field_name( 'new_window' ); ?>" value="1" <?php checked( 1, $instance['new_window'] ); ?>/> <?php esc_html_e( 'Open links in new window?', 'bridge' ); ?></label></p>
