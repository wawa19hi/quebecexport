	...
	class single_category_posts_widget extends WP_Widget {
		...
		public function __construct() {
			parent::__construct(
				...
				sprintf( __('%sCategory Posts', 'bridge' ), MTS_THEME_WHITE_LABEL ? '' : 'MTS ' ),
				array( 'description' => __( 'Display the most recent posts from a single category', 'bridge' ) )
			);
		}
	 	...
	 	public function form( $instance ) {
			...
			$title = isset( $instance[ 'title' ] ) ? $instance[ 'title' ] : __( 'Featured Category', 'bridge' );
			$title_length = isset( $instance[ 'title_length' ] ) ? intval( $instance[ 'title_length' ] ) : 15;
			$cat = isset( $instance[ 'cat' ] ) ? intval( $instance[ 'cat' ] ) : 0;
			$qty = isset( $instance[ 'qty' ] ) ? intval( $instance[ 'qty' ] ) : 3;
			$comment_num = isset( $instance[ 'comment_num' ] ) ? intval( $instance[ 'comment_num' ] ) : 0;
			$date = isset( $instance[ 'date' ] ) ? intval( $instance[ 'date' ] ) : 0;
			...
			?>
			<p>
				<label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php _e( 'Title:', 'bridge' ); ?></label>
				<input class="widefat" id="<?php echo $this->get_field_id( 'title' ); ?>" name="<?php echo $this->get_field_name( 'title' ); ?>" type="text" value="<?php echo esc_attr( $title ); ?>" />
			</p>
			<p>
				<label for="<?php echo $this->get_field_id( 'cat' ); ?>"><?php _e( 'Category:', 'bridge' ); ?></label>
			...
			<p>
				<label for="<?php echo $this->get_field_id( 'qty' ); ?>"><?php _e( 'Number of Posts to show', 'bridge' ); ?></label>
			...
			<p>
			   <label for="<?php echo $this->get_field_id( 'title_length' ); ?>"><?php _e( 'Title Length:', 'bridge' ); ?>
			...
			<p>
				<label for="<?php echo $this->get_field_id("show_thumb1"); ?>">
					...
					<?php _e( 'Show Thumbnails', 'bridge' ); ?>
				</label>
			</p>
			
			<p>
				<label for="<?php echo $this->get_field_id('box_layout'); ?>"><?php _e('Posts layout:', 'bridge' ); ?></label>
				<select id="<?php echo $this->get_field_id('box_layout'); ?>" name="<?php echo $this->get_field_name('box_layout'); ?>">
					<option value="horizontal-small" <?php selected($box_layout, 'horizontal-small', true); ?>><?php _e('Horizontal', 'bridge' ); ?></option>
					<option value="vertical-small" <?php selected($box_layout, 'vertical-small', true); ?>><?php _e('Vertical', 'bridge' ); ?></option>
				</select>
			</p>
			...
			<p>
				<label for="<?php echo $this->get_field_id("comment_num"); ?>">
					...
					<?php _e( 'Show number of comments', 'bridge' ); ?>
				</label>
			</p>
			...
			<p>
				<label for="<?php echo $this->get_field_id("show_excerpt"); ?>">
					...
					<?php _e( 'Show excerpt', 'bridge' ); ?>
				</label>
			</p>
			
			<p>
			   <label for="<?php echo $this->get_field_id( 'excerpt_length' ); ?>"><?php _e( 'Excerpt Length:', 'bridge' ); ?>
					...
					<div class="post-img">
						<a href="<?php echo esc_url( get_the_permalink() ); ?>" title="<?php echo esc_attr( get_the_title() ); ?>">
							<?php if ( has_post_thumbnail() ) { ?>
								<?php the_post_thumbnail( 'bridge-' . $thumbnail, array( 'title' => '' ) ); ?>
							<?php } else { ?>
								<img class="wp-post-image" src="<?php echo get_template_directory_uri() . '/images/nothumb-bridge-' . $thumbnail . '.png'; ?>" alt="<?php echo esc_attr( get_the_title() ); ?>"/>
							<?php } ?>
						</a>
					</div>
					<?php endif; ?>
