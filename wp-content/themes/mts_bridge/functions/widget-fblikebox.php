...
if( ! class_exists( 'Facebook_Like_Widget' ) ){
	class Facebook_Like_Widget extends WP_Widget {
		...
		{
			$widget_ops = array('classname' => 'facebook_like', 'description' => __('Add Facebook Like Box.', 'bridge' ));

			$control_ops = array('id_base' => 'facebook-like-widget');

			parent::__construct('facebook-like-widget', sprintf( __('%sFB Like Box', 'bridge' ), MTS_THEME_WHITE_LABEL ? '' : 'MTS ' ), $widget_ops, $control_ops);
		}
		...
		{
			$defaults = array('title' => __('Find us on Facebook', 'bridge' ), 'page_url' => '', 'width' => '292', 'show_faces' => 'on', 'show_stream' => false, 'show_header' => 'on');
			...
			<p>
				<label for="<?php echo $this->get_field_id('page_url'); ?>"><?php _e('Facebook Page URL', 'bridge' ); ?>:</label>
			...
			<p>
				<label for="<?php echo $this->get_field_id('width'); ?>"><?php _e('Width', 'bridge' ); ?>:</label>
			...
			<p>
				...
				<label for="<?php echo $this->get_field_id('show_faces'); ?>"><?php _e('Show faces', 'bridge' ); ?></label>
			</p>
			...
			<p>
				...
				<label for="<?php echo $this->get_field_id('show_stream'); ?>"><?php _e('Show stream', 'bridge' ); ?></label>
			</p>
			...
			<p>
				...
				<label for="<?php echo $this->get_field_id('show_header'); ?>"><?php _e('Show facebook header', 'bridge' ); ?></label>
			</p>
		<?php
		}
	}
