...
function rank_math_admin_notice() {
	...
	if ( ! is_admin() || ( 'dashboard' !== $current_screen->base ) || ! current_user_can( 'administrator' ) || get_option( 'rank_math_modules' ) ) {
		return;
	}
	...
	if ( ! $rm_installed ) {
		$action  = __( 'Install for FREE', 'bridge' );
		...
		$action  = __( 'Activate it now', 'bridge' );
	...
	<div class="notice rm-notice welcome-panel plugin-card-seo-by-rank-math" id="try-rankmath-panel" style="clear:both;">
		<div class="welcome-panel-content" style="overflow:hidden;">
			<img src="<?php echo esc_url( get_template_directory_uri() ); ?>/images/rank-math-banner.jpg" alt="Rank Math SEO" style="float: left;max-width: 315px;margin-right: 20px;margin-bottom: -60px;border: 5px solid #ededed;" />
			<div style="position: relative;">
				<h2><?php printf( __( 'Rank Math SEO plugin is recommended with %s theme!', 'bridge' ), '<strong>' . esc_attr( $theme_name ) . '</strong>' ); ?></h2>
				<p class="about-description"><?php echo esc_attr( 'The new Rank Math SEO plugin will help you rank better in the search results.', 'bridge' ); ?></p>
				<ul>
					<li>
						<i class="dashicons dashicons-yes"></i> <?php esc_html_e( 'Google Search Console Integration', 'bridge' ); ?>
					</li>
					<li>
						<i class="dashicons dashicons-yes"></i> <?php esc_html_e( 'Google Keyword Ranking', 'bridge' ); ?>
					</li>
					<li>
						<i class="dashicons dashicons-yes"></i> <?php esc_html_e( 'Google Keyword Suggession', 'bridge' ); ?>
					</li>
					<li>
						<i class="dashicons dashicons-yes"></i> <?php esc_html_e( '15+ Rich Snippets aka Schema Types', 'bridge' ); ?>
					</li>
					<li>
						<i class="dashicons dashicons-yes"></i> <?php esc_html_e( 'Advanced SEO Analysis Tool', 'bridge' ); ?>
					</li>
					<li>
						<i class="dashicons dashicons-yes"></i> <?php esc_html_e( 'Automated Image SEO', 'bridge' ); ?>
					</li>
					<li>
						<i class="dashicons dashicons-yes"></i> <?php esc_html_e( 'XML Sitemap', 'bridge' ); ?>
					</li>
					<li>
						<i class="dashicons dashicons-yes"></i> <?php esc_html_e( '404 Monitor', 'bridge' ); ?>
					</li>
					<li>
						<i class="dashicons dashicons-yes"></i> <?php esc_html_e( 'Redirection Manager', 'bridge' ); ?>
					</li>
					<li>
						<i class="dashicons dashicons-yes"></i> <?php esc_html_e( 'Local SEO Optimization', 'bridge' ); ?>
					</li>
					<li>
						<i class="dashicons dashicons-yes"></i> <?php esc_html_e( 'Lightweight', 'bridge' ); ?>
					</li>
					<li>
						<i class="dashicons dashicons-yes"></i> <?php esc_html_e( 'and much more...', 'bridge' ); ?>
					</li>
				</ul>
			...
			<a class="welcome-panel-close" id="wp_try_rankmath_panel-hide" href="<?php echo add_query_arg( 'rank_math_admin_notice_ignore', '1' ); ?>" aria-label="<?php esc_attr_e( 'Dismiss the Rank Math panel', 'bridge' ); ?>"><?php esc_html_e( 'Dismiss', 'bridge' ); ?></a>
		...
		<script type="text/javascript">
			jQuery(document).ready( function($) {
				...
				installRankmathSuccess = function( response ) {
					...
					tryRankmathPanel.find('.install-now').text('<?php echo esc_js( __( 'Activating...', 'bridge' ) ); ?>');
					setTimeout( function() { window.location.href = response.activateUrl; }, 100 );
				};
