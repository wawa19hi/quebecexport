...
function mts_inner_sidebar_metabox($post) {
	...
	*/
	...
	echo '<option value="" '.selected('', $custom_sidebar).'>-- '.__('Default', 'bridge' ).' --</option>';
	
	// Exclude built-in sidebars
	$hidden_sidebars = array('sidebar', 'footer-first', 'footer-first-2', 'footer-first-3', 'footer-first-4', 'footer-second', 'footer-second-2', 'footer-second-3', 'footer-second-4', 'widget-header','shop-sidebar', 'product-sidebar', 'widget-subscribe');	
	...
	echo '<option value="mts_nosidebar" '.selected('mts_nosidebar', $custom_sidebar).'>-- '.__('No sidebar --', 'bridge' ).'</option>';
	echo '</select><br />';
	
	// Select single layout (left/right sidebar)
	echo '<div class="mts_sidebar_location_fields">';
	echo '<label for="mts_sidebar_location_default" style="display: inline-block; margin-right: 20px;"><input type="radio" name="mts_sidebar_location" id="mts_sidebar_location_default" value=""'.checked('', $sidebar_location, false).'>'.__('Default side', 'bridge' ).'</label>';
	echo '<label for="mts_sidebar_location_left" style="display: inline-block; margin-right: 20px;"><input type="radio" name="mts_sidebar_location" id="mts_sidebar_location_left" value="left"'.checked('left', $sidebar_location, false).'>'.__('Left', 'bridge' ).'</label>';
	echo '<label for="mts_sidebar_location_right" style="display: inline-block; margin-right: 20px;"><input type="radio" name="mts_sidebar_location" id="mts_sidebar_location_right" value="right"'.checked('right', $sidebar_location, false).'>'.__('Right', 'bridge' ).'</label>';
	echo '</div>';
...
function mts_inner_posttemplate_metabox($post) {
	...
	*/
	...
	echo '<option value="" '.selected('', $posttemplate).'>'.__('Default Post Template', 'bridge' ).'</option>';
	echo '<option value="parallax" '.selected('parallax', $posttemplate).'>'.__('Parallax Template', 'bridge' ).'</option>';
	echo '<option value="zoomout" '.selected('zoomout', $posttemplate).'>'.__('Zoom Out Effect Template', 'bridge' ).'</option>';
	echo '</select><br />';
}
...
function mts_add_postheader_metabox() {
	...
	foreach ($screens as $screen) {
		add_meta_box(
			...
			__('Header Animation', 'bridge' ),	// title
...
function mts_inner_postheader_metabox($post) {
	...
	*/
	...
	echo '<option value="" '.selected('', $postheader).'>'.__('None', 'bridge' ).'</option>';
	echo '<option value="parallax" '.selected('parallax', $postheader).'>'.__('Parallax Effect', 'bridge' ).'</option>';
	echo '<option value="zoomout" '.selected('zoomout', $postheader).'>'.__('Zoom Out Effect', 'bridge' ).'</option>';
	echo '</select><br />';
}
