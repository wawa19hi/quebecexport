...
jQuery(document).ready(function() {
    ...
    $('.tabAdvancedWidgetOptionsTab').on('click',function(){
      ...
      $('#widgets li:nth-child('+(thisWidgetIndex+1)+')').children().children('.wdt-edit-controls').children('#updateWidgetAdvancedOps').click();
    ...
    $('.thisColumnWidgetsTab').on('click',function(){
      ...
      console.log( $('#'+pageBuilderApp.currentlyEditedColId).children('.thisColumnWidgetsTabTrigger') );

      $('#'+pageBuilderApp.currentlyEditedColId).children('.thisColumnWidgetsTabTrigger').click();
...
/* ----------- Live Text Editor Starts Here -------------- */
  ...
  $(document).on('click','.liveTextWrap',function(ev){
    ...
    if ( pageBuilderApp.isInlineSavingActive == true ) {
      ...
      $(' .wltControls ').mouseover(function(){
        $('.widgetHandle').css('display','none');
        $('.editColBtnTop').css('display','none');
      });
...
$('.card').click(function(ev){
  if ( $(ev.target).hasClass('updateTemplate') ) {
    ...
    $(this).append('<div id="updateTemplate" class="btn-green  updateTemplate" style=" padding:5px 30px; font-size:16px;"> Next </div>');
...
$('.popb_popup_close').click(function(){
...
// Save Triggers Start 
  ...
  jQuery('.rowCopyButton').click(function(){
    currentEditableRowID = jQuery('.currentEditingRow').val();
    jQuery('section[rowid="'+currentEditableRowID+'"]').children('#ulpb_row_controls').children('#copyThisRowLS').click();
  });
  jQuery('.rowPasteButton').click(function(){
    currentEditableRowID = jQuery('.currentEditingRow').val();
    jQuery('section[rowid="'+currentEditableRowID+'"]').children('#ulpb_row_controls').children('#pasteThisRowLS').click();
  });
  ...
  //col save triggers
  jQuery('.colOptionsFields , .popb_col_fields_container input , .popb_col_fields_container select , #columnBgColor').change(function(){
  ...
  jQuery('.pbp-widgets input, .pbp-widgets select, .pbp-widgets textarea').change(function(){
    saveWidgetTriggerOffside(this,'specific');
  });
  ...
  jQuery('.wdt-fields input , .wdt-fields select, .wdt-fields textarea , #widgetBgColor , .widgetStyling , .wdt-img input, .wdt-img select ').change(function(){
...
(function ($) {
  ...
  $('.templatesTypeFilterItem').on('click', function(){
    ...
    $('.template-card:contains("'+WidgetSearchQuery+'")').css('display','block');

    if (WidgetSearchQuery == 'All') {
      $('.template-card').css('display','block');
    }

    if (WidgetSearchQuery == 'Coming Soon') {
      $('.popup-tutorialNotice').show(300);
    }else{
      $('.popup-tutorialNotice').hide(300);
    }
...
(function($){
  ...
  $(document).ready(function() {
    ...
    jQuery('#addNewAccordionItem').click(function(){
  ...
  $(document).ready(function() {
    ...
    jQuery('#addNewtabItem').click(function(){
